...
let
    ...
    if builtins.isAttrs v && v ? doc && v ? value then
      # This is an mk result
      {
        ...
        combinators = 
          ...
          then lib.mapAttrs (_: extractDocs) v.value
          else {};
      }
  ...
  generateModuleMd = name: namespace: path:
    let
  ...
  allModuleDocs = lib.foldl' (acc: file:
    let
...
pkgs.runCommand "nfx-docs" {
  ...
  lib.concatStringsSep "\n" (
    [ "mkdir -p $out" "cp $summaryMdPath $out/SUMMARY.md" ] ++
