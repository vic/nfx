...
mk {
  ...
  value = rec {
      ...
      value =
        ...
        nfx.flatMap (
          ...
          if !step.more then
            ...
            nfx.flatMap (acc: fold.value acc f step.next) (f initial step.value)
        ) stream;
      tests = {
        "fold.value reduces to value" = {
          expr = nfx.runFx (
            fold.value 0 (acc: x: nfx.pure (acc + x)) (
              nfx.stream.fromList [
                1
                2
                3
    ...
    toList = mk {
      ...
      '';
      type = fn fx;
      value = stream: fold.value [ ] (acc: v: nfx.pure (acc ++ [ v ])) stream;
      tests = {
    ...
    forEach = mk {
      ...
      '';
      type = fn (fn fx);
      value = f: stream: fold.value { } (acc: v: nfx.map (_: acc) (f v)) stream;
      tests = {
        "forEach executes for each" = {
          expr = nfx.runFx (
            nfx.provide 0 (
              nfx.then' nfx.state.get (
                forEach (x: nfx.state.modify (s: s + x)) (
                  nfx.stream.fromList [
                    1
                    2
                    3
